...
let { is = useIs(), args, attach, manual = false, makeDefault = false, dispose, ref, oncreate, children, ...props } = $props();
// We can't create the object in a reactive statement due to providing context
let internalRef = $derived(determineRef(is, args));
// Create Event
const createEvent = useCreateEvent(oncreate);
