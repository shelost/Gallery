<svelte:head>
    ...
    <link href="https://fonts.googleapis.com/css?family=Abril+Fatface|Hammersmith+One|Righteous|Varela&display=swap" rel="stylesheet">
</svelte:head>
...
<script>
    ...
    onMount(() => {
        ...
        const SCALE = 2; // Resolution scaling factor
        ...
        // Load Audio
        var Audio = {
            funk: new window.Audio('/platformr/funk.mp3'),
            hiblip: new window.Audio('/platformr/blip_high.mp3'),
            loblip: new window.Audio('/platformr/blip_low.mp3'),
            succ: new window.Audio('/platformr/success.mp3')
        }

        // Set background music to loop
        Audio.funk.loop = true

        // Initialize audio (call this after user interaction)
        function initAudio() {
            if (!audioInitialized && !mute) {
                Audio.funk.play().catch(err => {
        ...
        // Load Images
        var logo = new Image()
        logo.src = '/platformr/platformr-logo.png'

        var buttonImg = new Image()

        var build = new Image()
        build.src = '/platformr/build.png'

        var move = new Image()
        move.src = '/platformr/move.png'

        var soundImg = new Image()

        var menuImg = new Image()
        menuImg.src = '/platformr/menu.png'

        var xImg = new Image()
        xImg.src = '/platformr/x.png'

        var title = new Image()
        title.src = '/platformr/title.png'

        var help = new Image()
        help.src = '/platformr/question.png'

        var guide = new Image()
        guide.src = '/platformr/build.png'

        // Helper functions to get logical dimensions
        ...
        function Resize(){
            ...
            // Reset transform and apply scale (important: canvas resize resets transform, but we explicitly set it)
            ctx.setTransform(1, 0, 0, 1, 0, 0); // Reset to identity
            ctx.scale(SCALE, SCALE);
            CENT_X = window.innerWidth/2;
            CENT_Y = window.innerHeight/2;
            GR_X = window.innerWidth/20;
            GR_Y = window.innerHeight/20;
            ONE = window.innerWidth*0.0012
        }
        ...
        function Restart(){
            if (level == 17){
                player.y = getCanvasHeight()
                player.x = getCanvasWidth()-40*ONE
            }else{
                player.y = 0
                player.x = 20*ONE
            }
        ...
        // Buttons
        var Button = new Squircle(getCanvasWidth()-GR_X*2.4, getCanvasHeight()-GR_X*2.4, GR_X*2,GR_X*2,GR_X)
        var startButton = new Squircle(CENT_X-GR_X*2, CENT_Y+GR_X*2, GR_X*4, GR_X*2, GR_X*0.8)
        var menuButton = new Squircle(getCanvasWidth()-GR_X*1.9, getCanvasHeight()-GR_X*1.5, GR_X*1.2,GR_X*1.2,GR_X*0.6)
        var helpButton = new Squircle(GR_X*0.7, getCanvasHeight()-GR_X*1.5, GR_X*1.2,GR_X*1.2,GR_X*0.6)
        var MenuX = new Squircle(GR_X, GR_X, GR_X, GR_X, GR_X)

        var muteButton = new Squircle(getCanvasWidth()-GR_X*2, GR_X, GR_X*1.2,GR_X*1.2,GR_X*0.6)

        // Start & End Blocks
        var Start = new Squircle(GR_X/2, CENT_Y, GR_X*1.25,GR_X/4,GR_X/8, false)
        blocks.push(Start)

        var End = new Squircle(getCanvasWidth()-GR_X*1.75, CENT_Y, GR_X*1.25, GR_X/4,GR_X/8,false)
        blocks.push(End)
        ...
        // Initialize Lava
        var lava = new Squircle(0,getCanvasHeight()-20,getCanvasWidth(), 20, 0)
        lava.up = 0
        ...
        const loop = ()=>{
            ...
            if (mute){
                soundImg.src = '/platformr/mute.png'
            }else{
                soundImg.src = '/platformr/sound.png'
            }
            ...
            if (level == 17){
                ...
                lava.y = getCanvasHeight()-20*ONE-lava.up
            }
            ...
            switch (level){
                ...
                case 17:
                    End.x = GR_X/2,
                    Start.x = getCanvasWidth()-GR_X*1.75
                    break;
            }
            ...
            // Player Boundaries
            if (player.x < 0 ||
                player.x > getCanvasWidth()-player.w){
                player.xspeed *= -1
            }
            ...
            if (player.dead){
                Audio.hiblip.play().catch(err => console.log('Audio play prevented:', err))
                Restart()
            }
            ...
            if (RectRect(player, goal)){
                ...
                Audio.succ.play().catch(err => console.log('Audio play prevented:', err))
                Restart()
            }
            ...
            // Enemies
            if (enemRange){
                for (let y=enemRange[0];y<enemRange[1]+1;y++){
                    var enem = Enemies[y]
                    ...
                    if (enemRange[0] == 45){ // The Shower
                        if (enem.y > getCanvasHeight()+enem.r){
                            enem.y = -enem.r
                        }

                        if (enem.x < -enem.r){
                            enem.x = getCanvasWidth()+enem.r
                        }
                    }else if (enemRange[0]== 73){ // The Climb
                        if (enem.y >getCanvasHeight()-enem.r-lava.h){
                            enem.y = getCanvasHeight()-enem.r-lava.h*1.2
                            enem.yspeed *= -1
                        }
                       ...
                       if (enem.y < enem.r){
                        ...
                        if (enem.x >getCanvasWidth()-enem.r ||
                            enem.x < enem.r){
                            enem.xspeed *= -1
                        }
                    ...
                    switch (level){
                        ...
                        case 9:
                                if (blocks.length-2 > y-16){
                                    ...
                                    enem.r = 0
                                   enem.x = Math.random()*getCanvasWidth()-GR_X*4+GR_X*2
                                   enem.y = getCanvasHeight()-GR_X
                                }
                                break;
            ...
            player.draw()
             ...
             // Player Trail
            for(let r=0;r<player.history.length; r++){
            ...
            if (scene == 3){
                ...
                ctx.fillText(`${limit-blocks.length+2} Block Left`, getCanvasWidth()-175*ONE, 40*ONE)
                }else{
                ctx.fillText(`${limit-blocks.length+2} Blocks Left`, getCanvasWidth()-175*ONE,40*ONE)
                }
            ...
            if (scene == 1){
                ...
                if (Touch(startButton)){
                    ...
                    initAudio() // Initialize and start background music
                    setTimeout(()=>{
                        scene = 3
                    },100)
            ...
            if (scene == 2){
                ...
                for (let i=0;i<grid.length;i++){
                    for (let j=0;j<grid[0].length;j++){
                        ...
                        if (Touch(rect) ){
                            ...
                            initAudio() // Initialize audio on level select
                            level = grid[i][j]
                            Restart()
                            setTimeout(()=>{
                                scene = 3
                ...
                if (Touch(MenuX)){
                    xImg.src = '/platformr/x-active.png'
                    setTimeout(()=>{
                        scene = 3
                    },100)
                }else{
        ...
        const BG_SCALE = 2; // Background resolution scaling factor

        function getBgWidth() {
            return bg.width / BG_SCALE;
        }

        function getBgHeight() {
            return bg.height / BG_SCALE;
        }

        function resizebg(){
            bg.width = window.innerWidth * BG_SCALE;
            bg.height = window.innerHeight * BG_SCALE;
            // Reset transform and apply scale
            bgx.setTransform(1, 0, 0, 1, 0, 0); // Reset to identity
            bgx.scale(BG_SCALE, BG_SCALE);
        }
        ...
        for (let u=0;u<20;u++){
            var c = new Circ(Math.random()*window.innerWidth, Math.random()*window.innerHeight, Math.random()*window.innerWidth/30+window.innerWidth/30)
            Circs.push(c)
        }

        const bgloop = () =>{
            bgx.clearRect(0,0,getBgWidth(), getBgHeight())
            ...
            bgx.fillRect(0,0,getBgWidth(), getBgHeight())
            ...
            for (let t=0;t<Circs.length;t++){
                ...
                if (circ.x > getBgWidth()+circ.r){
                    circ.x = -circ.r
                }

                if (circ.y > getBgHeight()+circ.r){
                    circ.y = -circ.r
                }
            }
        }
...
<canvas id="bg" style="position: fixed; top: 0px; left: 0px; z-index: 1"></canvas>
<canvas bind:this={canvas} id="canvas" style="position: fixed; top: 0px; left: 0px; z-index: 2"></canvas>
