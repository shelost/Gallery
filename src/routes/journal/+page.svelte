<script>
	...
	let journal = data.posts.filter(post => post.meta?.type == 'journal');
	let visible = false;

	onMount(() => {
		console.log('Journal page mounted');
	...
	onDestroy(() => {
		//console.log('Journal page destroyed');
		// Reset visible flag when leaving the page
		//visible = false;
	});

	function navigateToJournalPost(slug) {
...
<svelte:head>
	<title>Journal</title>
	<meta name="description" content="Journal entries by Heewon" />
</svelte:head>
...
{#if visible}
	<div class="text-column">
		...
		{#if data.posts && data.posts.length > 0}
			<div class="posts">
				{#each journal as post, i (post.slug)}
					{#if post.meta?.type == 'journal' && visible}
						<div class="post"
							on:click={() => navigateToJournalPost(post.slug)}
						...
						>
							<hgroup>
								...
								<p>{post.meta?.blurb || post.meta?.description}</p>
							</hgroup>
							<div class="content">
